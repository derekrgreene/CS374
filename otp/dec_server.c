...
void decrypt(char* plaintext, int textLength, char* key, int keyLength) {
  ...
  for (int i = 0; i < textLength; i++) {
    ...
    encryptedChar = (charVal - keyVal + 27) % 27;
    if (encryptedChar == 26) {
      plaintext[i] = ' ';
    } else {
...
int main(int argc, char *argv[]) {
  ...
  char *plaintext, *ack, *keyAck, *key, *handshake = "dec_server";
  ...
  while(1) {
    ...
    switch(spawnPid) {
      ...
      case 0:
        ...
        plaintext = malloc(plaintextSize);
        ...
        decrypt(plaintext, plaintextSize, key, keySize);
